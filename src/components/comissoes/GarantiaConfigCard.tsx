...
// =============================================================================
// GarantiaConfigCard — comissão de GARANTIA (Gestauto/seguros), por PRODUTO:
//   • Loja paga → cortesia → sem comissão (liga/desliga).
//   • Cliente paga → comissão FIXA por produto (gerente / vendedor).
//   • Produto não cadastrado → usa o default.
// A AutoConf entrega o PRODUTO real (ex.: "+150EX 2anos"); casamos por trecho do
// nome. `valor cobrado` é informativo. Reusa /api/commissions/garantia-config.
// =============================================================================
...
interface Produto { match: string; valorCobrado: string; gerente: string; vendedor: string }
interface Config { active: boolean; lojaPagaSemComissao: boolean; produtos: Produto[]; defaultGerente: string; defaultVendedor: string }
...
export default function GarantiaConfigCard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setCfg({
        ...
        defaultGerente: asText(d.defaultGerente), defaultVendedor: asText(d.defaultVendedor),
        produtos: (d.produtos ?? []).map((p: { match: string; valorCobrado: number | null; gerente: number; vendedor: number }) => ({ match: p.match, valorCobrado: asText(p.valorCobrado), gerente: asText(p.gerente), vendedor: asText(p.vendedor) })),
      })
  ...
  const addProd = () => setCfg((c) => c ? { ...c, produtos: [...c.produtos, { match: '', valorCobrado: '', gerente: '', vendedor: '' }] } : c)
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        defaultGerente: pnum(cfg.defaultGerente), defaultVendedor: pnum(cfg.defaultVendedor),
        produtos: cfg.produtos.map((p) => ({ match: p.match.trim(), valorCobrado: pnumOrNull(p.valorCobrado), gerente: pnum(p.gerente), vendedor: pnum(p.vendedor) })),
      }
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-card">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="flex items-center gap-2 text-base font-semibold text-gray-900"><ShieldCheck size={17} className="text-brand-600" />Comissão de garantia (por produto)</h2>
          <p className="mt-0.5 text-xs text-gray-500">Loja paga = cortesia. Cliente paga = comissão fixa por produto. O produto vem real do AutoConf.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50">
                <tr>{['Produto (trecho do nome)', 'Valor cobrado (info)', 'Gerente (R$)', 'Vendedor (R$)', ''].map((h) => <th key={h} className="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {cfg.produtos.map((p, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-2 py-1.5"><input inputMode="decimal" value={p.valorCobrado} onChange={(e) => setProd(i, 'valorCobrado', e.target.value)} placeholder="Ex: 3350" className={inputNum} /></td>
                    ...
                    <td className="px-2 py-1.5"><input inputMode="decimal" value={p.vendedor} onChange={(e) => setProd(i, 'vendedor', e.target.value)} placeholder="0" className={inputNum} /></td>
                ...
                {cfg.produtos.length === 0 && <tr><td colSpan={5} className="px-3 py-4 text-center text-sm text-gray-400">Sem produtos — usa o default abaixo.</td></tr>}
              </tbody>
            </table>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-700">Default gerente (produto sem cadastro)</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-700">Default vendedor (produto sem cadastro)</label>
              <input inputMode="decimal" value={cfg.defaultVendedor} onChange={(e) => { setSaved(false); setCfg({ ...cfg, defaultVendedor: e.target.value }) }} placeholder="0" className={inputNum} />
            </div>
          </div>
          <p className="text-xs text-gray-400">O produto é casado por trecho do nome (sem acento/caixa). Ex.: cadastre “150EX 2anos” → casa “Gestauto - +150EX 2anos”.</p>
