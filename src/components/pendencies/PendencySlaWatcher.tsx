...
// =============================================================================
// PendencySlaWatcher — pop-up BLOQUEANTE de SLA/Crítica. Ao entrar (e a cada
// intervalo) consulta /api/pendencies/action-required e exige ação:
//   • commit   — Alta/Urgente sem prazo: "Em quanto tempo você resolve isso?"
//   • charge   — Urgente com prazo estourado: "Você disse que resolveria até X…"
//   • critical — Crítica nível 2+: atualize o status / registre resposta / peça ajuda
// Cada exibição/resposta grava evento na timeline. Uma pendência por vez.
// (O banner fixo do nível 1 fica no PendencyCriticalBanner.)
// =============================================================================
...
type Kind = 'commit' | 'charge' | 'critical'
interface Item { kind: Kind; pendency: Pendency; decision?: Decision; level?: number }
...
export default function PendencySlaWatcher() {
  ...
  const fetchNext = useCallback(async () => {
    try {
      ...
      const popups = (j?.data?.popups ?? []) as Array<{ pendency: Pendency; decision: Decision }>
      const critical = (j?.data?.critical ?? []) as Array<{ pendency: Pendency; level: number }>
      const blocking: Item[] = [
        ...popups.map((p) => ({ kind: p.decision.kind, pendency: p.pendency, decision: p.decision })),
        ...critical.filter((c) => c.level >= 2).map((c) => ({ kind: 'critical' as const, pendency: c.pendency, level: c.level })),
      ]
      setItem((cur) => cur ?? blocking[0] ?? null)
    } catch { /* silencioso */ }
  }, [])
  ...
  useEffect(() => {
    ...
    fetch(`/api/pendencies/${item.pendency.id}/sla-action`, {
      ...
      body: JSON.stringify({ action: 'shown', kind: item.kind === 'charge' ? 'charge' : 'commit' }),
    }).catch(() => {})
  }, [item])
  ...
  if (!item) return null
  const { pendency: p } = item
  const isCharge = item.kind === 'charge'
  const isCritical = item.kind === 'critical'
  const headerBg = isCritical ? 'bg-red-700' : isCharge ? 'bg-red-600' : 'bg-orange-500'
  const title = isCritical ? 'PENDÊNCIA CRÍTICA' : isCharge ? 'COBRANÇA — prazo estourado' : 'DEFINA UM PRAZO PARA RESOLVER'
  const requiresResponse = isCharge || isCritical
  ...
  return (
    <div className="fixed inset-0 z-[9998] flex items-center justify-center p-4" role="alertdialog" aria-modal="true">
      ...
      <div className="relative z-10 flex max-h-[90dvh] w-full max-w-md flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className={cn('flex items-center gap-2 px-5 py-4 text-white', headerBg)}>
          {isCritical ? <Flame size={18} /> : isCharge ? <AlertTriangle size={18} /> : <Clock size={18} />}
        ...
        <div className="flex-1 space-y-3 overflow-y-auto px-5 py-4">
          <div className="rounded-xl border border-gray-200 p-3">
            <div className="flex items-center gap-2">
              <span className={cn('rounded px-1.5 py-0.5 text-[10px] font-semibold', isCritical ? 'bg-red-100 text-red-700' : PRI_COLOR[p.priority] ?? 'bg-gray-100 text-gray-600')}>{isCritical ? 'CRÍTICA' : p.priority}</span>
        ...
        <div className="flex flex-col gap-2 border-t border-gray-100 px-5 py-3">
          <button onClick={requiresResponse ? handleRespond : handleCommit} disabled={busy} className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50">
            {busy ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> : <CalendarClock size={15} />}
            {requiresResponse ? 'Registrar resposta' : 'Registrar prazo'}
          </button>
          <div className="flex items-center justify-between">
            ...
            {item.kind === 'commit' && item.decision?.canDefer && (
