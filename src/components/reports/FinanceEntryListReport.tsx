...
export default function FinanceEntryListReport({
  ...
  const [from, setFrom] = useState(''); const [to, setTo] = useState(''); const [q, setQ] = useState('')

  const load = useCallback(async () => {
    setLoading(true)
    try {
      const qs = new URLSearchParams({ view }); if (from) qs.set('from', from); if (to) qs.set('to', to); if (q) qs.set('q', q)
