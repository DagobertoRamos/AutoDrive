...
// =============================================================================
// AtendimentosPanel — histórico de atendimentos do dia, com ações de gestão
// (reabrir/cancelar/excluir). Usado como aba dentro de Relatórios da Fila.
// =============================================================================
...
export default function AtendimentosPanel({ from, to }: { from?: string; to?: string } = {}) {
  ...
  const manage = async (id: string, action: 'reopen' | 'cancel' | 'delete' | 'finish') => {
    const labels = { reopen: 'reabrir', cancel: 'cancelar', delete: 'EXCLUIR', finish: 'finalizar' }
                ...
                <tr key={a.id} className="hover:bg-gray-50">
                  ...
                  {canManage && (
                    <td className="whitespace-nowrap px-4 py-3 text-right">
                      {['CALLED', 'ACCEPTED', 'IN_ATTENDANCE'].includes(a.status) && <button onClick={() => manage(a.id, 'finish')} disabled={busy === a.id} className="inline-flex rounded p-1 text-green-600 hover:bg-green-50 disabled:opacity-40" title="Finalizar"><XCircle size={14} className="rotate-45" /></button>}
