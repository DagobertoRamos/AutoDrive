...
// O título não vai por cima da arte: as artes já trazem o texto; ele serve de
// texto alternativo. Pausa com o mouse em cima.
...
export function SiteBannerCarousel({ banners, intervalSeconds }: { banners: SiteBanner[]; intervalSeconds: number }) {
  ...
  return (
    <div className="banner-carousel" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      {b.linkUrl
        ? <a href={b.linkUrl} target={b.newTab ? '_blank' : undefined} rel={b.newTab ? 'noreferrer' : undefined} className="banner-slide">{img}</a>
