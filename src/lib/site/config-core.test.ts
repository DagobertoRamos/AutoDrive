...
describe('config do site', () => {
  ...
  it('sanitiza slug, domínios, cores, links e serviços', () => {
    const c = sanitizeSiteConfig({
      ...
      services: { estoque: false, sobre: false, atacado: true, seoLandings: true, bogus: true },
    }, 'Loja')
    ...
    expect(serviceOn(c, 'seoLandings')).toBe(true)
    expect((c.services as Record<string, boolean>).bogus).toBeUndefined()
