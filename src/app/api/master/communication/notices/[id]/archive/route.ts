...
export async function POST(_req: NextRequest, ctxArg: { params: { id: string } | Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (notice.status === 'ARCHIVED') {
      return NextResponse.json({ success: false, error: 'Aviso já está arquivado.' }, { status: 400 })
    }
    const now = new Date()
    const updated = await prisma.internalNotice.update({
      where: { id: params.id },
      data:  { status: 'ARCHIVED', active: false, archivedAt: now, updatedById: session.id },
    })
    await prisma.internalNoticeLog.create({ data: { noticeId: params.id, userId: session.id, action: 'ARCHIVED', details: { previousStatus: notice.status } } }).catch(() => {})
    return NextResponse.json({ success: true, data: updated, message: 'Aviso arquivado.' })
