...
export async function POST(req: Request, { params }: { params: Promise<{ site: string }> }) {
  ...
  try {
    ...
    if (existing) {
      ...
      return NextResponse.json({ success: true, protocol: existing.leadNumber ? `#${existing.leadNumber}` : null })
    }
    ...
    return NextResponse.json({ success: true, protocol: leadNumber ? `#${leadNumber}` : null }, { status: 201 })
  } catch (err) {
