// =============================================================================
// API: /api/settings/email — AutoDrive
// Configuração SMTP para envio de e-mails
//
// Chave de armazenamento: `t:{tenantId}:email.{field}`
// =============================================================================
...
export async function GET(_req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: Object.keys(config).length > 0 ? config : null })
  } catch (err) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, message: 'Configuração de e-mail salva.' })
  } catch (err) {
    console.error('[POST /api/settings/email]', err)
    return NextResponse.json({ success: false, error: 'Erro interno' }, { status: 500 })
  }
}
