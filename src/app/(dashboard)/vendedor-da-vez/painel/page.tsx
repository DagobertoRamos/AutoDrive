  ...
  const load = useCallback(async () => {
    try {
      ...
      const [cRes, aRes, atRes, pvRes] = await Promise.all([
        ...
        fetch(`/api/seller-queue/customer-arrivals${sp}`, { credentials: 'include' }),
        fetch(`/api/seller-queue/attendances?active=true${sp ? `&${sp.slice(1)}` : ''}`, { credentials: 'include' }),
        fetch(`/api/seller-queue/pos-vendas${sp}`, { credentials: 'include' }),
      ])
      fetch(`/api/seller-queue/events?limit=80${sp ? `&${sp.slice(1)}` : ''}`, { credentials: 'include' }).then((r) => r.ok ? r.json() : null).then((j) => { if (j?.success) setEvents(j.data ?? []) }).catch(() => {})
      fetch(`/api/seller-queue/callable${sp}`, { credentials: 'include' }).then((r) => r.ok ? r.json() : null).then((j) => { if (j?.success) setCallable(j.data ?? []) }).catch(() => {})
